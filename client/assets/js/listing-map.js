function initMap() {
  var mapEl = document.getElementById("mapCanvasTwo");
  if (!mapEl) return;
  var map;
...
// Load initialize function only when Google Maps API is available and map element exists
function initMapWhenReady() {
  if (typeof google === 'undefined' || typeof google.maps === 'undefined') {
    setTimeout(initMapWhenReady, 50);
    return;
  }
  if (!document.getElementById('mapCanvasTwo')) return;
